/**
 * WarriorProtectionStrategy - Auto-generated ai_strategy
 *
 * Auto-generated by TrinityCore MCP Server
 * Generation date: 2025-11-01T10:01:47.771Z
 ...
 */
class WarriorProtectionStrategy : public CombatStrategy {
public:
    WarriorProtectionStrategy();
    virtual ~WarriorProtectionStrategy() = default;
...
WarriorProtectionStrategy::WarriorProtectionStrategy() {
    // Initialize strategy
}

void WarriorProtectionStrategy::Update(BotAI* ai, uint32 diff) {
...
bool WarriorProtectionStrategy::ShouldEngage(Unit* target) const {
...
Unit* WarriorProtectionStrategy::SelectTarget(BotAI* ai) const {
    Player* bot = ai->GetBot();
...
bool WarriorProtectionStrategy::IsValidTarget(Unit* target) const {
    return target &&
           !target->IsDead() &&
           target->IsInWorld() &&
...
uint32 WarriorProtectionStrategy::GetNextAbility(BotAI* ai) const {
    Player* bot = ai->GetBot();
    Unit* target = ai->GetCurrentTarget();
...
std::vector<WarriorProtectionStrategy::AbilityPriority> WarriorProtectionStrategy::BuildRotation(BotAI* ai) const {
...
bool WarriorProtectionStrategy::ShouldUseDefensiveCooldown(BotAI* ai) const {
    Player* bot = ai->GetBot();
    return bot->HealthBelowPct(30);
}

bool WarriorProtectionStrategy::ShouldUseOffensiveCooldown(BotAI* ai) const {
    Unit* target = ai->GetCurrentTarget();
    if (!target)
        return false;
...
bool WarriorProtectionStrategy::HasEnoughMana(Player* bot, uint32 spellId) const {
...
bool WarriorProtectionStrategy::IsSpellReady(Player* bot, uint32 spellId) const {
    return bot->HasSpell(spellId) &&
           !bot->HasSpellCooldown(spellId) &&
           HasEnoughMana(bot, spellId);
}

float WarriorProtectionStrategy::CalculateThreat(Unit* target) const {
    // Simplified threat calculation
    return target->GetThreat(ai->GetBot());
}


void WarriorProtectionStrategy::ManageThreat(BotAI* ai, Unit* target) const {
...
bool WarriorProtectionStrategy::NeedThreatBoost(BotAI* ai) const {
    Unit* target = ai->GetCurrentTarget();
    if (!target)
        return false;
...
uint32 WarriorProtectionStrategy::GetThreatSpell() const {
