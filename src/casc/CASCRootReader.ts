...
export class CASCRootReader {
  ...
  parseRootFile(data: Buffer, locale: string = 'enUS'): void {
    process.stderr.write(`[CASCRootReader] Parsing root file (${data.length} bytes)
`);
    ...
    try {
      ...
      process.stderr.write(`[CASCRootReader] Root file signature: ${signature}
`);

      if (signature === 'TVFS') {
        process.stderr.write(`${'[CASCRootReader] Using TVFS format parser (modern WoW builds)'}
`);
        this.parseTVFSFormat(data, locale);
      } else if (signature === 'MNDX') {
        process.stderr.write(`${'[CASCRootReader] Using MNDX format parser'}
`);
        this.parseMNDXFormat(data, locale);
      } else {
        // Try parsing as legacy format
        process.stderr.write(`${'[CASCRootReader] Using legacy format parser'}
`);
        this.parseLegacyFormat(data, locale);
      }

      process.stderr.write(`[CASCRootReader] Loaded ${this.totalEntries} root entries, ${this.entries.size} unique paths
`);
      logger.info('CASCRootReader', `Loaded ${this.totalEntries} root entries, ${this.entries.size} unique paths`);
    } catch (error) {
      process.stderr.write(`[CASCRootReader] ERROR parsing root file: ${error}
`);
  ...
  private parseLegacyFormat(data: Buffer, locale: string): void {
    process.stderr.write(`${'[CASCRootReader] Parsing legacy format root file'}
`);
    ...
    while (offset < data.length - 12) {
      try {
        ...
        if (count === 0 || count > 1000000) {
          process.stderr.write(`[CASCRootReader] Invalid count ${count} at offset ${offset}, stopping
`);
          break;
        }
        ...
        const matchesLocale = (localeFlags & localeFlag) !== 0 || localeFlags === LocaleFlag.ALL;
        process.stderr.write(`[CASCRootReader] Block ${blocksProcessed}: count=${count}, matches locale=${matchesLocale}
`);
        blocksProcessed++;
        ...
        process.stderr.write(`[CASCRootReader] Error parsing legacy block at offset ${offset}: ${error}
`);
    ...
    process.stderr.write(`[CASCRootReader] Legacy format: processed ${blocksProcessed} blocks, ${this.totalEntries} entries added
`);
  }
  ...
  private parseTVFSFormat(data: Buffer, locale: string): void {
    process.stderr.write(`${'[CASCRootReader] Parsing TVFS format root file'}
`);
    ...
    try {
      ...
      process.stderr.write(`[CASCRootReader] TVFS Header: FormatVersion=${header.formatVersion}, EKeySize=${header.eKeySize}, PathTableSize=${header.pathTableSize}, VfsTableSize=${header.vfsTableSize}, CftTableSize=${header.cftTableSize}
`);
      ...
      process.stderr.write(`[CASCRootReader] TVFS Tables extracted: pathTable=${pathTable.length}b, vfsTable=${vfsTable.length}b, cftTable=${cftTable.length}b
`);

      // Parse the path/file tree
      this.parseTVFSPathTable(pathTable, vfsTable, cftTable, header);

      process.stderr.write(`[CASCRootReader] TVFS format: parsed ${this.totalEntries} entries
`);
    } catch (error) {
      process.stderr.write(`[CASCRootReader] Error parsing TVFS format: ${error}
`);
      throw error;
    }
  }
  ...
  private parseTVFSPathFileTable(pathTable: Buffer, vfsTable: Buffer, cftTable: Buffer, header: any, startOffset: number, pathBuffer: string[], maxOffset?: number): number {
    ...
    while (offset < endBoundary) {
      ...
      if (offset >= pathTable.length) {
        process.stderr.write(`[CASCRootReader] TVFS parser reached end of pathTable at offset ${offset}
`);
        break;
      }
      ...
      if (pathEntry.hasNodeValue) {
        if ((pathEntry.nodeValue & 0x80000000) !== 0) {
          ...
          if (dirLen <= 0 || dirLen > pathTable.length) {
            process.stderr.write(`[CASCRootReader] Invalid folder dirLen: ${dirLen}
`);
            break;
          }
          ...
          if (folderEndOffset > pathTable.length) {
            process.stderr.write(`[CASCRootReader] Folder end offset ${folderEndOffset} exceeds buffer
`);
            break;
          }
    ...
    try {
      ...
      if (this.listFile && this.listFile.isLoaded()) {
        ...
        if (this.totalEntries < 20) {
          process.stderr.write(`[CASCRootReader] FileDataID conversion: "${filePath}" → decimal ${parsedFileDataId}, listFile=${this.listFile ? 'loaded' : 'NULL'}
`);
        }
        ...
        if (parsedFileDataId > 0) {
          ...
          if (resolvedPath) {
            ...
            if (this.totalEntries < 10) {
              process.stderr.write(`[CASCRootReader]   → Resolved to: "${resolvedPath}" (FileDataID: ${fileDataId})
`);
            }
          } else {
            ...
            if (this.totalEntries < 10) {
              process.stderr.write(`[CASCRootReader]   → NOT in listfile, using FileDataID: ${actualFilePath}
`);
            }
          }
        } else {
          ...
          if (this.totalEntries < 10) {
            process.stderr.write(`[CASCRootReader]   → FileDataID is 0, skipping
`);
          }
          return;
        }
  ...
  private addEntry(entry: CASCRootEntry): void {
    ...
    if (entry.fileId !== undefined) {
      ...
      if (this.fileIdMap.size <= 10) {
        process.stderr.write(`[CASCRootReader] ✅ Added FileDataID ${entry.fileId} → "${entry.filePath}" to fileIdMap (size: ${this.fileIdMap.size})
`);
      }
    } else {
      // Debug: Log first 5 entries WITHOUT FileDataID
      if (this.totalEntries < 5) {
        process.stderr.write(`[CASCRootReader] ⚠️  Entry WITHOUT FileDataID: "${entry.filePath}"
`);
      }
    }
  ...
  setListFile(listFile: CASCListFile): void {
    ...
    process.stderr.write(`[CASCRootReader] ListFile set (${listFile.getEntryCount()} entries)
`);
  }
