...
function findDeallocationForVariable(
  ...
  for (const func of ast.symbols.functions.values()) {
    if (func.location.line <= allocLine && (func.location?.endLine ?? func.location.line) >= allocLine) {
      if (
...
function hasSmartPointerNullCheck(ast: AST, variable: VariableSymbol, usageLine: number): boolean {
  ...
  for (const func of ast.symbols.functions.values()) {
    if (func.location.line <= usageLine && (func.location?.endLine ?? func.location.line) >= usageLine) {
      const checkPatterns = [
        `if (!${variable.name})`,
            ...
            if (bodyAfter && (bodyAfter.includes(`${name}->`) || bodyAfter.includes(`*${name}`))) {
              violations.push({
                ...
                suggestedFix: {
                  ...
                  diff: `@@ -${func.location.line},1 +${func.location.line},2 @@
 ${pattern}(${name});
+${name} = nullptr;`,
                  codeSnippet: {
                    before: `${pattern}(${name});`,
          ...
          if (hasOpen && !hasClose) {
            violations.push({
              ...
              suggestedFix: {
                type: 'custom',
                file: context?.file,
                line: func.location?.endLine ?? func.location.line - 1,
                explanation: `Add ${name}.Close() before function exit`,
                diff: `@@ -${func.location?.endLine ?? func.location.line - 1},0 +${func.location?.endLine ?? func.location.line - 1},1 @@
+    ${name}.Close();`,
                codeSnippet: {
                  before: `${name}.Open();\n// ... use resource\n}`,
                  after: `${name}.Open();\n// ... use resource\n    ${name}.Close();\n}`,
